...
# Name of the file. Keep this script and file in same folder.
filename = 'test'
...
# for value in sheet.iter_rows(min_row=starting_rows, max_row= no_of_candidates, min_col=starting_column, max_col=max_attributes, values_only=True):
#     print(value)

for rows in range(starting_rows, no_of_candidates):
    ...
    for columns in range(starting_column, max_attributes):
        if(count == 1):
            ...
            if(drill_wr > drill_wr_total * pass_ind) and (drill_pr > drill_pr_total * pass_ind) and (drill_tr > drill_total * pass_ind) and (writing_wr > writing_wr_total * pass_ind) and (writing_pr > writing_pr_total * pass_ind) and (writing_tr > writing_total * pass_ind) and (misc_tr > misc_total* pass_ind) and (special_wr > special_wr_total * pass_ind) and (special_pr > special_pr_total * pass_ind) and (spl_tr > special_total* pass_ind) and (grand_total > total * pass_ttl):
                if(grand_total > 375):
                    temp = sheet.cell(row = rows, column = max_attributes-1)
                    temp.value = 'A'
                    # var = 'Y' + str(rows)
                    # sheet[var] = "A"
                ...
                # var = 'Y' + str(rows)
                # sheet[var] = "Fail"
            # if(drill_wr > drill_wr_total * pass_ind):
            #     if(drill_pr > drill_pr_total * pass_ind):
            #         if(drill_tr > drill_total * pass_ind):
            #             if(writing_wr > writing_wr_total * pass_ind):
            #                 if(writing_pr > writing_pr_total * pass_ind):
            #                     if(writing_tr > writing_total * pass_ind):
            #                         if(misc_tr > misc_total * pass_ind):
            #                             if(special_wr > special_wr_total * pass_ind):
            #                                 if(special_pr > special_pr_total * pass_ind):
            #                                     if(spl_tr > special_total * pass_ind):
            #                                         if(grand_total > total * pass_ttl):
            #                                             is_pass = 1
            # if(is_pass == 1):
            #     if(grand_total > 375):
            ...
            #         var = 'Y' + str(rows)
            #         sheet[var] = "A"
            #     elif(grand_total > 300) and (grand_total < 374):
            #         # temp = sheet.cell(row = rows, column = max_attributes)
            #         # temp.value = 'B'
            #         var = 'Y' + str(rows)
            #         sheet[var] = "B"
            #     elif(grand_total > 250) and (grand_total < 299):
            #         # temp = sheet.cell(row = rows, column = max_attributes)
            #         # temp.value = 'C'
            #         var = 'Y' + str(rows)
            #         sheet[var] = "C"
